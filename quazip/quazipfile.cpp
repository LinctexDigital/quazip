...
void QuaZipFilePrivate::setZipError(int zipErrorLocal) const
{
  QuaZipFilePrivate *fakeThis = const_cast<QuaZipFilePrivate*>(this); // non-const
  fakeThis->zipError=zipErrorLocal;
  if(zipErrorLocal==UNZ_OK)
    q->setErrorString(QString());
  else
    q->setErrorString(QuaZipFile::tr("ZIP/UNZIP API error %1").arg(zipErrorLocal));
}
